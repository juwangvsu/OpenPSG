...
def test_model_forward_and_psgtr_bootstrap(monkeypatch) -> None:
    ...
    class PsgtrForPanopticSceneGraphGeneration(nn.Module):
        def __init__(self, config):
            ...
            self.model.backbone.conv_encoder = nn.Linear(3, 3)
            self.model.backbone.position_embedding = nn.Linear(3, 3)
            self.transformer_marker = nn.Linear(config.d_model, config.d_model)
    ...
    assert bootstrapped.bootstrap_report["initialization"] == "full_psgtr_pretrained"
    assert bootstrapped.lidar_query_fusion.gate.item() == 0.0

    source = PsgtrForPanopticSceneGraphGeneration.from_pretrained("checkpoint")
    backbone_only = (
        model_module.PsgLidarEnhForPanopticSceneGraphGeneration
        .from_psgtr_backbone_pretrained("checkpoint")
    )
    assert backbone_only.bootstrap_report["initialization"] == (
        "pretrained_image_backbone_only"
    )
    assert torch.equal(
        backbone_only.model.backbone.conv_encoder.weight,
        source.model.backbone.conv_encoder.weight,
    )
    assert not torch.equal(
        backbone_only.subject_bbox_embed.weight,
        source.subject_bbox_embed.weight,
    )
